...
def flow_to_image(flow):
    ...
    return tf.py_func(np_flow_to_image, [flow], tf.uint8, stateful=False)
...
def axangle2mat(axis):
    ...
    Returns
    -------
    mat : array shape (BS, 3, 3)
       rotation matrix for specified rotation
    Notes
    -----
    From: http://en.wikipedia.org/wiki/Rotation_matrix#Axis_and_angle
    '''
    bs = axis.shape.as_list()[0]
    x, y, z = axis[:, 0], axis[:, 1], axis[:, 2]
    angle = tf.norm(axis, axis=1, keep_dims=True)
    axis /= angle
    c = tf.cos(tf.squeeze(angle))
    s = tf.sin(tf.squeeze(angle))
    C = 1.0 - c
    xs = x*s;   ys = y*s;   zs = z*s
    xC = x*C;   yC = y*C;   zC = z*C
    xyC = x*yC; yzC = y*zC; zxC = z*xC
    r_mat = tf.stack([x*xC+c,   xyC-zs,   zxC+ys,
                      xyC+zs,   y*yC+c,   yzC-xs,
                      zxC-ys,   yzC+xs,   z*zC+c,], axis=1)
    return tf.reshape(r_mat, [bs, 3, 3])
